...
export class AddNoteComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (!this.subjectId) {
            ...
            this.notesService.getSubject(this.subjectId).subscribe(res => {
                this.subject = res;
                this.createForm();
                this.isLoading = false;
    ...
    onSave() {
        ...
        if (this.subjectId) {
            this.notesService.addNote({'subject_id': this.subjectId, 'body': newBody}).subscribe(res => {
            ...
            // if the subject the user types is an existing subject
                ...
                this.notesService.addNote({'subject_id': subject_id, 'body': newBody}).subscribe(res => {
                    if (this.sharedOrNot == 'mysubject') {
                        this.router.navigateByUrl(`/notes/subject/${subject_id}`).then(() => {
                            this.toasterService.showToaster('Created Successfully!', '', 3000);
                        });
                    } else if (this.sharedOrNot == 'subjectshared') {
                        this.router.navigateByUrl(`/notes/subject_shared/${subject_id}`).then(() => {
                            this.toasterService.showToaster('Created Successfully!', '', 3000);
                        });
                    }
                });
            ...
            // if it is a new subject
                this.notesService.addSubject({'name': subject}).subscribe(res => {
                    this.notesService.addNote({'subject_id': res.id, 'body': newBody}).subscribe(res1 => {
                        if (this.sharedOrNot == 'mysubject') {
                            this.router.navigateByUrl('/notes/subject/' + res.id).then(() => {
                                this.toasterService.showToaster('Created Successfully!', '', 3000);
                            });
                        } else if (this.sharedOrNot == 'subjectshared') {
                            this.router.navigateByUrl('/notes/subject_shared/' + res.id).then(() => {
                                this.toasterService.showToaster('Created Successfully!', '', 3000);
                            });
                        }
                    });
