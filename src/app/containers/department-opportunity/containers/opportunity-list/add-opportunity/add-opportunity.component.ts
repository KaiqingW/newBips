      ...
      if (this.addSubjectForm.valid && this.saveOnce) {
          ...
          var request = {
            //   'name': this.addSubjectForm.value.name, 
              ...
              'subject_type' : 'sales_department_opportunity',
          ...
          this.departmentOpportunityService.addSubject(this.companyId, request).subscribe(              
              res => {
                ...
                // get the added subject id, used for upload attachment
                let newAddedSubjectId = res.id;

                // when add department opportunity, we need to add two default processes
                ...
                this.fd.append('category', 'SKU Creation');
                this.fd.append('description', 'text/plain');

                this.businessMeetingService.addSubjectAttachment(this.companyId, newAddedSubjectId, this.fd).subscribe(
                    (res) => {                
                        this.fd.append('category', 'QUOTE');
                    
                        this.businessMeetingService.addSubjectAttachment(this.companyId, newAddedSubjectId, this.fd).subscribe(
                            res => {
                                this.isLoading = false;
                                // after add opportuniy, directly route to the subject
                                this.router.navigateByUrl(`/company/${this.companyId}/department-opportunity/opportunity-subject/${newAddedSubjectId}`);
